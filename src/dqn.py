...
class DQN:
    def __init__(self,
        ...
        self.next_x_batch = tf.placeholder(tf.float32, shape=[self.batch_size] + self.state_shape)
        ...
        y_j = self.rewards + self.gamma * tf.multiply(self.done_mask, tf.reduce_max(q_next_outputs, axis=1))
    ...
    def _policy(self, x):
        ...
        ff1 = tf.layers.dense(conv2, units=256, activation=tf.nn.relu, name="ff1")
